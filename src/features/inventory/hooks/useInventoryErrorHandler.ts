    ...
    try {
      ...
      if (productsError) {
        ...
        products.forEach(product => {
          ...
          const difference = Math.abs(calculatedStock - product.stock_quantity);
          if (difference > 0.001) { // Tolerância para arredondamentos
            errors.push(`Inconsistência no produto ${product.name}: estoque atual ${product.stock_quantity}, calculado ${calculatedStock}`);
          }
        });
      }
