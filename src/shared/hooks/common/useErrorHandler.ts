    ...
    try {
      ...
      if (finalConfig.logError) {
        try {
          ...
          // Falha silenciosa no logging para não quebrar o sistema
        }
      }
      ...
      if (finalConfig.showToast) {
        try {
          ...
          // Fallback: log simples se toast falhar
          console.warn('Error handler: Toast failed, original error:', error);
        }
      }
      ...
      //   } catch (sentryError) {
      //     // Falha silenciosa no Sentry
      //   }
      // }
    } catch (handlerError) {
      // Último recurso: log básico se tudo falhar
      console.warn('Error handler failed:', handlerError, 'Original error:', error);
    }
