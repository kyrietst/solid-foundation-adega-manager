...
export const useNotifications = () => {
  ...
  const { data: lowStockProducts = [], isLoading: isLoadingLowStock } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('products')
        .select('id, name, stock, min_stock, category, supplier')
        .lt('stock', 'min_stock')
  ...
  const { data: outOfStockProducts = [], isLoading: isLoadingOutOfStock } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('products')
        .select('id, name, stock, category, supplier')
        .eq('stock', 0)
  ...
  const notifications = useMemo((): NotificationItem[] => {
    ...
    lowStockProducts.forEach(product => {
      items.push({
        ...
        type: 'low_stock',
        title: 'Estoque Baixo',
        message: `${product.name} - ${product.stock} restantes (mínimo: ${product.min_stock})`,
        priority: 'medium',
        read: false,
