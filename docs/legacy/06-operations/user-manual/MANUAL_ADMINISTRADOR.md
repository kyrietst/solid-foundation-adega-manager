...
**Versão 2.7.0 | Sistema de Gestão de Adega Wine Cellar**
**📅 Última atualização:** 08 de novembro de 2025
**🆕 Novidades:** Sistema de Reset de Senha Modernizado + Edge Functions
...
#### **Resetar Senha de Usuário (✨ Novo Sistema - v3.5.0):**

**Como resetar a senha de qualquer usuário:**
1. Vá em **Usuários** no menu lateral
2. Localize o usuário na lista
3. Clique no botão **"Resetar Senha"** (ícone de chave 🔑)
4. O sistema irá:
   - Gerar automaticamente uma senha temporária de 8 caracteres
   - Exibir a senha em um modal para você copiar
   - Configurar o usuário para OBRIGATORIAMENTE trocar a senha no próximo login

**⚠️ IMPORTANTE - O que acontece após o reset:**
- A senha temporária é exibida **UMA ÚNICA VEZ**
- **Copie e envie imediatamente** ao usuário (via WhatsApp, email, etc.)
- Quando o usuário fizer login:
  1. Sistema aceita a senha temporária normalmente
  2. Um modal **BLOQUEIA o acesso** exigindo troca de senha
  3. Usuário **NÃO CONSEGUE usar o sistema** sem trocar a senha
  4. Após definir nova senha, acesso é liberado normalmente

**🔐 Segurança:**
- ✅ Apenas administradores podem resetar senhas
- ✅ Senha temporária válida por tempo indeterminado (mas DEVE ser trocada)
- ✅ Sistema força troca obrigatória no primeiro login
...
Resetei sua senha de acesso ao sistema Adega Manager.

Senha temporária: A95YVF80

No próximo login, o sistema vai pedir para você criar uma senha nova.
...
**Troubleshooting:**
- **Modal não apareceu após login:** Contate o administrador do sistema
- **Senha temporária não funciona:** Verifique se copiou corretamente (8 caracteres)
- **Esqueci a senha nova que criei:** Peça novo reset ao administrador

#### **Editar Permissões:**
- **Ativar/Desativar:** Controlar acesso ao sistema
- **Alterar Tipo:** Promover ou rebaixar função (Admin/Funcionário/Entregador)
- **Resetar Senha:** Sistema seguro com Edge Function (detalhes acima)

#### **Monitorar Atividade:**
- **Último Login:** Quando acessou pela última vez
...
R: Como administrador, você pode resetar sua própria senha através de outro administrador pelo menu **Usuários → Resetar Senha**. Se for o único administrador, contate o suporte técnico.

**P: Como funciona o novo sistema de reset de senha (v3.5.0)?**
R: O sistema agora usa **Edge Functions** para maior segurança:
- Admin clica "Resetar Senha" no usuário
- Sistema gera senha temporária de 8 caracteres automaticamente
- Usuário OBRIGATORIAMENTE troca senha no primeiro login
- Senha SERVICE_ROLE nunca é exposta no navegador
- Apenas administradores conseguem resetar senhas de outros usuários

**P: Um funcionário esqueceu a senha. E agora?**
R:
1. Vá em **Usuários** no menu lateral
2. Encontre o funcionário na lista
3. Clique em **"Resetar Senha"**
4. Copie a senha temporária exibida
5. Envie para o funcionário via WhatsApp ou email
6. No próximo login, ele será FORÇADO a criar uma senha nova

**P: O que acontece se o usuário não trocar a senha temporária?**
R: O sistema **BLOQUEIA o acesso** com um modal que não pode ser fechado. O usuário não consegue usar nenhuma funcionalidade até criar uma senha nova de pelo menos 8 caracteres.

**P: Posso ter mais de um administrador?**
R: Sim, mas crie com cuidado. Administradores têm acesso total ao sistema, incluindo resetar senhas de outros administradores.
