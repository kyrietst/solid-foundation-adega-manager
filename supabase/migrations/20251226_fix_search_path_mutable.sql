...
BEGIN
    FOR func_record IN
        ...
        WHERE n.nspname = 'public' 
          ...
          AND p.proowner = (SELECT oid FROM pg_roles WHERE rolname = 'postgres' OR rolname = 'service_role' OR rolname = 'authenticated' OR rolname = 'anon' OR rolname = current_user) -- Filter to owned functions if needed, but public schema usually implies user functions
    LOOP
        BEGIN
            EXECUTE format(
